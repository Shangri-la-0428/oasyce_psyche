...
v9.2.5 在这个基础上继续做两件事：一是把关系动力学和持续主体偏置接进热路径，二是把回应调节做成可观测、可回看的闭环，而不是单向建议。
...
### 2. AI-first ABI + 双回应 profile (`subjectivity.ts`, `response-contract.ts`, `host-controls.ts`)
...
Historically, v9 was framed as an "Inverse Blade Runner" buildout. The more precise v9.2.5 framing is:
...
### 4. AI-first control surface + reply profiles
...
### 5. Closed-loop metacognition (`metacognition.ts`)
...
### 6. Layered semantic memory (`psyche-file.ts`)
...
user input
  ...
  → apply chemistry + merge subject residue
  → autonomic / processing-depth gating
  → policy + subjectivity kernel + response contract + generation controls
